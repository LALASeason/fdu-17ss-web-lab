...
<form id="f_upload" enctype="multipart/form-data" method="post" name="form" action="uploadSong.php">
    <p>请上传音乐文件</p>

    <!--TODO: 在这里补充 html 元素，使 file_upload 上传后若为音乐文件，则可以直接播放-->
    <audio controls preload = "auto">
        <P>The browser can't support the audio!</P>
    </audio>

    <input type="file" name="file_upload" onchange="upload()">
    ...
    <table>
        ...
        <tr><td><input type="button" value="插入时间标签" onclick="insertTimeTag()"></td><td><input type="button" value="替换时间标签" onclick="replaceTimeTag()"></td></tr>
...
<section id="s_show" class="content">
    <form name = "displayTheForm">
    <select name = "nameOfSong" onchange = "changeSong()">
    <!--TODO: 在这里补充 html 元素，使点开 #d_show 之后这里实时加载服务器中已有的歌名-->
        <option>Please select a song~</option>
        <?php
        $process = opendir(dirname(__FILE__) . "/files" . ".");
        $fileArray = array();
        while (($file = readdir($process))!= false ){
            if ($file != "." && $file != ".." && strrchr(strtolower($file), ".mp3") == ".mp3"){
                $fileArray[] = $file;
            }
        }

        closedir($process);
        foreach ($fileArray as $i -> $music){
    ...
    <audio controls preload="auto">
        <p>The browser doesn't support the audio!</p>
    </audio>
    <button type="button" onclick="previousSong()">上一首</button>
    <button type="button" onclick="nextSong()">下一首</button>

    </form>
    <div id="displayTheLyrics">
        <ul id="lyrics" style="margin-top: 240px"></ul>
    </div>
</section>
</body>
<script>
...
function upload() {
    document.getElementsByTagName("audio")[0].src = window.URL.createObjectURL(document.form.file_upload.files[0]);
}

function getTime() {
    let time = document.getElementsByTagName("audio")[0].currentTime;
    let minute = parseInt((time / 60).toString());
    time -= minute * 60;
    minute = standardNumber(minute);
    let second = parseInt(time.toString());
    time -= second;
    second = standardNumber(second);
    time = parseInt((time * 100).toString());
    time = standardNumber(time);
    let theTime = "[" + minute + ";" + second + "." + time + "]";
    return theTime;
}

function standardNumber(num) {
    if (num.toString().length === 1){
        let number = ("0" + num).toString();
        return number;
    }
}
...
function insertTimeTag(){
    let value = document.getElementById("edit_lyric").value;
    document.getElementById("edit_lyric").value = value.substring(0,get_target_pos()) + getTime() + value.substring(get_target_pos(), value.length);
}

function replaceTimeTag() {
    let value = document.getElementById("edit_lyric").value;
    let replaceValue = value.substring(get_target_line()[0], get_target_line()[1]);
    document.getElementById("edit_lyric").value = value.replace(replaceValue.substring(0,10), getTime());

}

let selectElement = document.form.name;
